<?PHP
	...
	# Where both need different layouts and dynamic side contents
	...
	# if cookie not there	
	if(!@$_COOKIE['meta_keywords']){		
		$PV['COOKIE'] = $SG->set_master_cookie();		 
	}
	...
	if(is_file($LIB_PATH."/inc/".$PAGE.".php")){
		...
		#print_r($page_info);
	        ...
	        $page_info	=  dynamic_page_content(array('manipulation' => " code = '$PAGE'  " ));
	...
	if($PAGE=='home'){
		...
		$CONTENT[$PAGE]['layout'] = 'layout_home';
		$CONTENT[$PAGE]['add_on'] = get_add_on_content($rdsql,$G);
	}
	...
	function page_content($content){
		...
		if(!is_file($content['lib_path']."/inc/$page.php")){		
			...
			$page 			= (is_file("template/page_content/$page.html"))?$page:'home';
			
			$c 			= new Template(array("filename" => "template/page_content/$page.html",
							    "debug"    => 0));
	...
	function dynamic_page_content($param){
			...
			$select_data = "SELECT
						...
						image_a as page_header_image,
						image_b as page_right_image						
					FROM
						entity_child
					WHERE
						$param[manipulation]
	...
	// get add on content
	function get_add_on_content($rdsql,$G){
		
		
		
		// get profile
		
		//$select_tot = "SELECT entity_value as total_profile FROM entity_key_value WHERE entity_key='total_profile'";
		// 
		//$exe_tot_row = $rdsql->exec_query($select_tot,'---');
		//
		//$get_tot_row = $rdsql->data_fetch_object($exe_tot_row);
		
		//$home_address = json_decode($G->get_one_columm(array('field'	=> 'concat(\'{\',group_concat(concat(\'"\',sn,\'":"\',detail,\'"\')),\'}\')',
		//						 'table'	=> 'entity_child',
		//						 'manipulation' => 'WHERE entity_code="MP" and code ="PLHN"'
		//						)
		//					)
		//			);
		//return array(						   
		//		//'TOTAL_PROFILE'    => $get_tot_row->total_profile,
		//		//'MARQUEE'          => add_on_data(array('entity_code'=>'MQ' )),//$INFO,
		//		////'HOME_BANNER'      => add_on_data(array('entity_code'=>'HB')),//$INFO,				
		//		////
		//		//// home address detail
		//		//
		//		'HOME_ADD_NAME'    => $home_address->orgname,
		//		'HOME_ADD_I'       => $home_address->add1,
		//		'HOME_ADD_II'      => $home_address->add2,				
		//		'HOME_ADD_PHONE'   => $home_address->phone,
		//		'HOME_ADD_MOBILE'  => $home_address->mobile,				
		//		'HOME_ADD_EMAIL'   => $home_address->email,
		//);
	...
	function add_on_data($param){
		
		global $rdsql;
		
		$select_marquee = "SELECT
					sn,
					ln,
					image
				FROM
					entity_child
				WHERE
					entity_code = '$param[entity_code]' AND is_active=1";
		$exe_marquee 	= $rdsql->exec_query($select_marquee,'error sub menu ');
			
		$INFO = array();
		
		while($get_row = $rdsql->data_fetch_object($exe_marquee)){
		    
		      $temp = array();
		      $temp['TITLE'] = $get_row->sn;
		      $temp['URL']   = $get_row->ln;
		      $temp['image']   = $get_row->image;	
	...
	function menu_create($param){
			 ...
			 while($get_row	 = $rdsql->data_fetch_object($exe_select_info)){
				 ...
				 $temp['menu_code'] =  (!$get_row->redirect)?'?'.$get_row->code:$get_row->redirect;
