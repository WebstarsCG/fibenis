...
################################################################################################################
		# Check For Db Backup
		    ...
		    #Check for Full db backup
				...
				my $backup_db_txt = $db_path->file("db_.txt");
			...
			if (exists($backup_r_v->{db_backup}->{delete_table}) && (@{$backup_r_v->{db_backup}->{delete_table}->{table_to_delete}})){
			   ...
			   # my $d_database_name =   $backup_r_v->{delete_table}->{database_name};
				...
				if ($hash_key[$group_key] eq 'trigger_table') {
				    	
				    `cd $mysql_bin && mysqldump --no-data -h localhost -u $user_name $password --skip-triggers $data_base > $p_d_table_s_file_name`;
				    
				    `cd $mysql_bin && mysqldump -h localhost -u $user_name $password --no-create-info --skip-triggers $data_base > $p_d_table_file_name `;
				    
				    my $trigger=$db_path."trigger.sql";
				    
				    `cd $mysql_bin && mysqldump --routines=true  -u $user_name $password  $data_base > $trigger`;
				    
				    #`cd $mysql_bin && mysqldump --routines --no-create-info --no-data --no-create-db --skip-opt $data_base > $trigger`;
...
#                        
    ...
    # Restore function automatic
		...
		#    }
		    ...
		    my $tigger_path_structure   		=   $path_db.'/trigger_table_group_table_exclude_structure_backup_db_backup.sql';
		    
		    my $tigger_path_data	  		=   $path_db.'/trigger_table_group_table_exclude_data_backup_db_backup.sql';
		    
		    print "gokul--->".$tigger_path_structure;
		    
		    my $trigger					=   $path_db.'/trigger.sql';
		...
		#print "gokul trigger";
		    ...
		    $dbh->do( "use $auto_db");
                    ...
                    `cd $mysql_bin && mysql -u $backup_user_name $backup_password $auto_db < $tigger_path_structure `;
                    
                    `cd $mysql_bin && mysql -u $backup_user_name $backup_password $auto_db < $tigger_path_data `;
		    
		    `cd $mysql_bin && mysql -u $backup_user_name $backup_password $auto_db < $trigger `;
		    ...
		    &FILE_WRITE::txt_file($temp_v);
                    ...
                    print "\n";
                    print "Trigger DB BACKUP \n\n";
                    print "\tSussfully Update \n" ;
