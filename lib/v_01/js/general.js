...
function crypt_url(url){	
    ...
    fetch((q_index==-1)?url+'?PASS_ID='+sess_id:url+'&'+'PASS_ID='+sess_id+'&page_axn=1',
	{
		method: 'GET', // or 'PUT'
		headers: {
	...
	.then((html) => {
		...
		var temp_content =document.getElementById('desk_data').innerHTML;
		
		document.getElementById('desk_data').innerHTML=html;
		
		if(Number(G.$('pager_records').dataset.pager)==0){
			
			console.log(G.$('pager_records').dataset.pager+'--');
			...
			var JS_CAll =new Object({'show_user_info':show_user_info,
									...
									'd_series.set_inline_update':d_series.set_inline_update,
