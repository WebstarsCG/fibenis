...
function crypt_url(url){	
	...
	// check action change
	lv.is_d_axn_change = (d_series.isDeskActionChange()==true)?1:0;
 
    fetch((q_index==-1)?url+'?PASS_ID='+sess_id:url+'&'+'PASS_ID='+sess_id+'&page_axn=1&axn_change='+lv.is_d_axn_change,
	{
		method: 'GET', // or 'PUT'
		headers: {
	...
	.then((html) => {
		...
		const pager_start = (G.$('page').dataset.prevStatus!=G.$('page').value)?0:G.$('pager_action').dataset.pager;
		
		set_page_status(Number(G.$('pager_records').dataset.pager),
						Number(G.$('page').value),
						Number(G.$('pager_action').dataset.pager));
		
		G.$('page').dataset.prevStatus = G.$('page').value;
		
						//console.log('JS Calls--'+js_calls);	
		var JS_CAll =new Object({'show_user_info':show_user_info,
								'd_series.inline_on_off':d_series.inline_on_off,
							    'd_series.set_nd':d_series.set_nd,
								'boolean_display':boolean_display,
								'show_user_info_2l':show_user_info_2l});

		const matches = document.querySelectorAll("td[data-js]");
		for(var element of matches){

			console.log('---'+element.dataset.js);	

			element.innerHTML = JS_CAll[element.dataset.js](element.dataset.p);
		}
