				 ...
				 for(var x in lv.check_elements){					    
					    ...
					    if(lv.check_elements[x].is_active==true){
						       ...
						       if (document.getElementById(x)!=undefined) {
								  console.log('Hide'+document.getElementById(x).className);
								  //code
						       ...
						       if(lv.current_element.tagName!="DIV") {
								  ...
								  if(lv.current_element.dataset.inputType=='FT'){
									     ...
									     if(alphanum_clean(lv.current_element.value).length==0){
											...
											f_series.setInputMessage[lv.ft]({"element":lv.current_element,
															 "message":get_style({"style":"block txt_size_13 clr_red","content":"Kindly select any avilable option."}),
															 "color"  :"error"});
									     }
									     else if(lv.current_element.tagName!="SELECT"){
												
											console.log(lv.current_element.dataset.isFormat);	
												   
											if(lv.current_element.dataset.isFormat!=undefined){
												   
												   console.log('R'+checkIsFormat(lv.current_element));
		     ...
		     lv.heading_keys = Object.keys(lv.refill_headings);
		      ...
		      for (var hx in lv.heading_keys) {
				 ...
				 if(lv.temp_element!=false) {
					    ...
					    //code
					    lv.temp_message=(lv.refill_headings[lv.heading_keys[hx]]==0)?'':'&nbsp;&nbsp;&nbsp;(&nbsp;<b class="txt_size_15">'+lv.refill_headings[lv.heading_keys[hx]]+'</b> inputs seems not filled fully. Kindly fill it.)';
					    
					    lv.ft = G.isUndefined(lv.temp_element.form_type,'FS');
					    
					    lv.display_message  = (lv.temp_message.length==0)?'':((lv.temp_element.dataset.headingType=='TB')?get_style({"style":"txt_size_12 normal clr_red","content":'&nbsp;&#9888;&nbsp;'}):get_style({"style":"txt_size_11 normal clr_red","content":lv.temp_message}));
...
function crypt_url(url){	
   ...
   //var is_internal = document.getElementById('internal').value;
   
   //alert(is_internnal);
   
   //document.location.href=(q_index==-1)?url+'?PASS_ID='+sess_id+'&internal='+is_internal:url+'&'+'PASS_ID='+sess_id+'&internal='+is_internal;
...
///////////////////////  FORM GENERAL VALUS ////////////////////////////////////////////
	   ...
	   function check_all(element_count,element_prefix){
		      ...
		      for(var i=1;i<=element_count;i++){
				 ...
				 if(D.getElementById(element_prefix+i).disabled==false){										;
