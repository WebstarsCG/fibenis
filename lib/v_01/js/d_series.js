					...
					if(param.length>0){	
						...
						if(Object.keys(lv.param).length!=0){
							  ...
							  if( (lv.param.is_fa) && (!lv.param.is_fa_btn)){
										 ...
										 document.write('<a href="JavaScript:d_series.call_nd('+encodeURI(param)+')" class="ficon hint--left" data-hint="'+lv.param.link_title+'"><i class=" fa '+lv.param.is_fa+'" aria-hidden="true"></i>'+lv.title+'</a>');           
							  }
							  else if( (lv.param.is_fa) && (lv.param.is_fa_btn)){
										 
										 document.write('<a  class="btn '+lv.param.is_fa_btn+''+this.css.dynamic_label+' hint--bottom"  href="JavaScript:d_series.call_nd('+encodeURI(param)+')"  data-hint="'+lv.param.title+'" >'+
															  '<i class=" fa '+lv.param.is_fa+'"></i>'+
															  '<span id='+lv.param.token+'>'+lv.param.link_title+'</span></a>');           
							  }
							  else{
							 
										 document.write('<a href="JavaScript:d_series.call_nd('+encodeURI(param)+')">'+lv.param.link_title+'</a>');           
							  }
						} // end
					}
           } // end
	   ...
	   D_Series.prototype.nd_close=function(wall_id){
           ...
           D_Series.prototype.set_inline=function(param){
                      ...
                      document.write(lv.txtbox+lv.hidden);
           } // end
           ...
           D_Series.prototype.set_inline_update=function(param){
                      ...
                      document.write(lv.input+lv.hidden);
