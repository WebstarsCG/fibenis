 ...
 */
	...
	function send_mail($param){
		...
		if(is_array($param['from']) 
			&& array_key_exists('title',$param['from'])
			&& array_key_exists('mail',$param['from'])){
				
			$mail->setFrom($param['from']['mail'],
			               $param['from']['title']);
		...
		foreach($param['to'] as &$to){ $mail->addAddress($to);}			
	...
	function mail_send_smtp($param){					
		...
		return $mail_sent = send_mail($mail); // send mail internal call	
