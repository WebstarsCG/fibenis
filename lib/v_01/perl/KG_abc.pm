#!/usr/bin/perl -w                               
    ...
    # Creating PDF File Function
        ...
        # get the inner array datas
                    ...
                    for(@data_in){
                        ...
                        $c_v{data_value}->{'line_height'}= $c_v{data}->{$c_v{st_var}}->{line_height};
                    ...
                    $c_v{rt_xaxis}=$c_v{data}->{right_xaxis} || ($c_v{width})-($c_v{page}->{margin}->{right});
    ...
    # Draw the horizontal line
                ...
                prAdd("q $h_v{x} $h_v{ul} m $h_v{rt_xaxis} $h_v{ul} l S Q") if(!$h_v{data}->{hide_top_line});        # draw upper horizontal line
