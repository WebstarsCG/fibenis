#!/usr/bin/perl -w
    ...
    # Page Variable
		...
		# Data Neutral
			...
			$e_series->{'data_neutral'} = {};
					
			$l_v{'data_neutral_counter'}=0;
					
			for(@{$e_series->{'data'}}){
				
				$e_series->{'data_neutral'}->{$_->{'id'}}=$_;
				
				$_->{'col_index'} = $l_v{'data_neutral_counter'};
				
				$l_v{'data_neutral_counter'}++;
			}
		
                print "Data Neutral".Dumper($e_series->{'data_neutral'}) if($param->{'is_debug'});
		...
		@{$l_v{'temp_dynamic_group'}}=$l_v{'type'}=~m/^(S|M)(\d+)/ig;
		
		$l_v{'dynamic_group'} = $e_series->{'data_neutral'}->{$l_v{'temp_dynamic_group'}->[1]};
		
		#  column		
		if ($l_v{'dynamic_group'} ) {
			$l_v{'group'}->{'column'}=$l_v{'dynamic_group'}->{'col_index'};
		}		
		
		print "Dynamic Group".Dumper($l_v{'dynamic_group'}) if($param->{'is_debug'});
		...
		$e_series->{'order'} = ($l_v{'dynamic_group'}->{'field'})? ' ORDER BY '.$l_v{'dynamic_group'}->{'field'}.
				...
				if($l_v{'num_of_rows'}){
						...
						if (!$l_v{'group'}->{'is_final_only'}){
							...
							@{$l_v{'final_row'}} = ($l_v{'group'}->{'final_row_action'})?@{$l_v{'group'}->{'final_row_action'}->({'label'  => $group_info->{NAME},
														        'column'=> $l_v{'dynamic_group'}->{'field'},
    ...
    # 02Jan2018 -> Group Index & Column mismatch solved. key_id matched to group id
    # Group Sample 
