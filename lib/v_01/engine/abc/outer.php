<?PHP
	...
	# Config & Header
		...
		$COACH['name']		=   (get_config('is_multiple')==1)?str_replace("www.","",$_SERVER['HTTP_HOST']):'default';
		
		if(!isset($_SESSION[$COACH['name']])){ $_SESSION[$COACH['name']]=md5($COACH['name']); };
		
		$COACH['name_hash']     =   $_SESSION[$COACH['name']];
		
		$COACH['path']          =  ((get_config('coach_path'))?get_config('coach_path'):"template/");
		
		$COACH['filter']        =  (get_config('is_multiple')==1)? " AND get_page_coach(id)='$COACH[name]' " :"";
	...
	# Home & Key URL setup	
		...
		// Menu Content
				...
				if(!$MENU_OFF){
						...
						$M->AddParam('PARENT_MENU',menu_create(array('manipulation'=>" WHERE 1=1 $COACH[filter] AND entity_code='PG' AND parent_id=0  ",
											     'is_home'     => $IS_HOME
											     ))
							    );
