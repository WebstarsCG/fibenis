<?PHP
    ...
    $F_SERIES	=	array(
		...
		$F_SERIES['after_add_update'] =function($key_id){
						...
						//attributemap
						if(is_object($xml->attributemap)){
							$lv['attrmap'] = (array)$xml->attributemap;
						}
							...
							$rdsql->exec_query("INSERT INTO entity(code,sn,is_lib,user_id) VALUES(".implode($lv['entity'],',').")",'insert entity');
							   ...
							   if(is_object($entity->attributes->attribute)){
							       ...
							       foreach($entity->attributes->attribute as $field_idx=>$field_obj){
								       ...
								       $rdsql->exec_query("INSERT INTO entity_child_base(entity_code,token,sn,ln,dna_code,user_id)
												  VALUES('".$entity->code."','".$lv['attr']['token']."','".$lv['attr']['name']."','".$lv['attr']['description']."','EBAT',$USER_ID)",'ecb');
								...
								//    echo "INSERT INTO ecb_av_addon_varchar(parent_id,ea_code,ea_value,user_id)
								//			   VALUES $lv[addon_query_text]";
								    
								    $lv['addon_query_text_neutral']=substr($lv['addon_query_text'],0,-1);
								    $rdsql->exec_query("INSERT INTO ecb_av_addon_varchar(parent_id,ea_code,ea_value,user_id)
											   VALUES $lv[addon_query_text_neutral]","ecb varchar");
